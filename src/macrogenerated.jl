endpoint_tuple = [
            ...
            (:post_oauth, :post_update_profile_image, "account/update_profile_image.json", []),
            (:post_oauth, :post_account_update_profile_banner, "account/update_profile_banner.json", []),
            #have arguments spliced into url: doesn't currently work
            # (:get_oauth, :get_geo_id_place_id, "geo/id/\$place_id.json", [:place_id]),
            # (:get_oauth, :get_user_suggestions_slug, "users/suggestions/\$slug.json", [:slug]),
            # (:get_oauth, :get_user_suggestions_slug_members, "users/suggestions/\$slug/members.json", [:slug]),
            # (:get_oauth, :get_retweets_id, "statuses/retweets/\$id.json", [:id]),
            # (:post_oauth, :post_destroy_single_tweet_id, "statuses/destroy/\$id.json", [:id]),
            # (:post_oauth, :post_status_retweet_id, "statuses/retweet/\$id.json", [:id]),

]

#dynamically build methods
for (verb, func, endp, spliced) in endpoint_tuple
    @eval begin
            function ($func)(;kwargs...)
                ...
                #Bring url spliced arguments into function scope, delete from dict
                # for sym in $spliced
                #     sym = options[string(sym)]
                #     delete!(options, string(sym))
                # end
